...
#   })
# }
# lumifi_sg Security Group for Lumifi Resources
resource "aws_security_group" "lumifi_sg" {
  name        = "${local.project_name.name}-${terraform.workspace}-lumifi_sg"
  description = "lumifi_sg security group for Lumifi resources in ${terraform.workspace}"
  vpc_id      = terraform.workspace == "prod" ? data.aws_vpc.existing[0].id : aws_vpc.lumifi-vpc[0].id
...
# Lambda Security Group - Common for both environments
resource "aws_security_group" "lambda_sg" {
  name        = "${local.project_name.name}-${terraform.workspace}-lambda"
  description = "Security group for Lambda functions in ${terraform.workspace}"
  vpc_id      = terraform.workspace == "prod" ? data.aws_vpc.existing[0].id : aws_vpc.lumifi-vpc[0].id

  # Allow all outbound traffic for Lambda
  egress {
